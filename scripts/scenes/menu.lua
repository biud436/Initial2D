...
-- 라벨 중앙 정렬용 절반 폭. GetTextWidth(1단계)로 실제 비트맵 폰트 폭을 재고,
-- 폰트가 아직 없으면 글자 수로 어림한다 (한글 글리프가 대략 30px).
local function labelHalfOf(label)
	if GetTextWidth ~= nil then
		local w = GetTextWidth(label)
...
local function makeButton(label, scene, cy)
	local x = W / 2 - BTN_W / 2
	local y = cy - BTN_H / 2
	return {
		label = label,
...
function MenuScene.init()
	...
	buttons = {
		makeButton("플래피 버드", "flappy", H / 2 - 170),
		makeButton("타일맵 데모", "tilemap", H / 2 - 58),
		makeButton("떠나기 전에", "title", H / 2 + 54),
		makeButton("알데바란", "aldebaran_title", H / 2 + 166),
	}
end
...
function MenuScene.update(elapsed)
	...
	icon.setPosition(W / 2 - 46, H / 2 - 250 + math.sin(t * 3.0) * 10.0)
	icon.update(elapsed)
	bg.update(0)
