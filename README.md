...
이 엔진으로 만든 횡스크롤 액션 게임입니다. 엔진 코드는 손대지 않았고, 물리와 충돌과 전투를 전부 `scripts/games/aldebaran/`의 Lua로 썼습니다.

보물 사냥꾼 카르토가 금단의 숲 알데바란에서 가면 원숭이에게 배낭과 지도를 빼앗기고, 그것을 되찾으러 검은 안개의 숲과 황제의 무덤을 지나는 이야기입니다. 스테이지는 둘이고, 1-1을 끝내면 레벨과 골드를 들고 1-2로 이어집니다. 기획서는 [docs/design/aldebaran.md](./docs/design/aldebaran.md)에 있습니다.
...
플래피 버드, 타일맵, 「떠나기 전에」 데모도 저장소에 남아 있습니다. 게임의 흐름에는 들어 있지 않고, `INITIAL2D_SCENE=flappy`처럼 이름을 주면 그 씬으로 바로 열립니다.
...
256타일에 구간이 다섯입니다. 구간 경계에서 배경 두 벌을 겹쳐 서서히 갈아 끼우고, 구간마다 흔적이 하나씩 있습니다.
...
흔적을 밟으면 글이 뜨고 힘이 하나씩 붙습니다. 검기, 흔적 읽기, 도약은 늘 켜져 있고 폭주와 검기 방출은 쿨타임이 있습니다. 흔적은 강제가 아니라 지나쳐도 스테이지는 끝나지만, 결과 창의 "알아낸 것"이 비고 에필로그의 마지막 줄이 붙지 않습니다.

## 스테이지 1-2 황제의 무덤

석회암 바위산을 스핑크스 모양으로 깎아 만든 무덤이고, 지면에 닿은 가슴께로 들어갑니다. 320타일이며 입구 복도와 방 넷으로 되어 있습니다.

방마다 기후가 다르고, 기후는 연출이 아니라 조작을 바꿉니다.
...
마지막 방의 아포피스는 3페이즈이고, 페이즈가 넘어갈수록 후딜이 1.2초에서 0.6초로 짧아집니다. 페이즈가 바뀌는 동안에는 잠깐 무적입니다.

## 몬스터

몬스터는 여덟입니다. 상태는 순찰 → 추적 → 공격으로 흐르고, 종별 차이는 코드가 아니라 `data/monsters.lua`의 표에 있습니다. 접촉 데미지는 없고 선딜레이가 보이는 공격만 아픕니다.
...
```bash
# 1-1: 카르토와 몬스터 시트, 타일셋, 원경, 타이틀, HUD, 효과음 8종
python3 tools/generate_aldebaran_assets.py

# 1-1 검은 안개의 숲 (256x28). 지형과 배치는 전부 손으로 정한 좌표입니다
python3 tools/generate_aldebaran_maps.py

# 1-2: 무덤의 시트 넷, 석회암 타일 32종, 방 다섯의 배경
python3 tools/generate_aldebaran_tomb.py

# 1-2 황제의 무덤 (320x30). 방과 복도, 구덩이 둘
python3 tools/generate_aldebaran_tomb_map.py
```

## 기획서에서 표 꺼내기 (tools/spica_extract.py)

알데바란의 바탕은 워드로 쓴 기획서 `docs/design/Spica_v0.9.docx`입니다. 이 파일은 저장소에 넣지 않으므로(`docs/design/.gitignore`), 안의 표와 문단과 이미지 목록을 마크다운으로 뽑아 두었습니다. 파이썬 표준 라이브러리만 씁니다.

```bash
python3 tools/spica_extract.py            # 마크다운 셋을 docs/design/spica-source/에 씁니다
python3 tools/spica_extract.py --media    # 이미지 17장도 꺼냅니다 (커밋 대상이 아닙니다)
python3 tools/spica_extract.py --check    # 세기만 하고 파일은 쓰지 않습니다
```

| 산출물 | 내용 |
| :--- | :--- |
| `spica-source/tables.md` | 표 58개. 찾아보기와 절 번호가 붙습니다 |
| `spica-source/text.md` | 문단 전부. 표와 그림은 자리만 표시합니다 |
| `spica-source/media.md` | 이미지 17장의 목록과 절 |
| `spica-unused.md` | 기획서의 무엇이 게임에 들어갔고 무엇이 남았는지, 남은 것은 어느 단계에서 쓸지 |

같은 docx면 같은 바이트가 나오므로 두 번 돌려도 `git diff`가 비어 있습니다. 이미지는 커밋하지 않지만 세계 지도 `image6.png` 한 장만 예외로 넣었습니다. 워드의 `.emf` 그림 넷은 안에 비트맵 한 장을 감싸고 있을 뿐이라, 그 비트맵을 꺼내 `sips`로 PNG를 만듭니다. libreoffice가 있으면 그쪽을 먼저 씁니다.

## 인수 테스트

`tests/engine/scenes/aldebaran_scene.lua`가 입력 재생기로 타이틀부터 1-1의 에필로그까지 매번 다시 통과합니다. 도입 컷씬, 게임 오버와 다시 하기, 대쉬와 턱과 다리, 전투와 성장, 체크포인트 부활, 짐도둑, 배낭, 그리고 1-2로 이어지는 것까지 봅니다. 전투 굴림이 시드 난수라 결과는 늘 같습니다.

1-2는 좌표를 박지 않은 자율 봇이 따로 주파합니다. 막히면 뛰고 적이 있으면 베는 것이 전부인데, 방 넷의 기후를 다 겪고 새 적 셋을 다 만나고 아포피스까지 쓰러뜨립니다.
