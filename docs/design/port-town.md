...
graph TD
    ...
    A --> B[광장: 게시판, 우물, 생선 장수]
    B -->|창고 사연| C[여관 실내: 주인이 창고 열쇠를 내준다]
    C -->|창고 열쇠| W[창고 문을 연다 → 등유 한 통]
    W -->|등유| E[언덕의 등대지기: 오늘 밤 불을 켠다, 은화 두 닢]
    E -->|은화 두 닢| R[여관: 방을 잡는다]
    B --> D[골목: 아이]
    E -.->|천공의 끝 이야기| D
    D -.->|조개 목걸이| F
    R --> F{부두의 배}
...
- **막힘 없이 5분**: 부두에서 배에 바로 말을 걸어도 끝낼 수 있다. 다만 선장이 "아직 해가
  ...
  **심부름 사슬은 강제가 아니라 "더 볼 것"이다.**
- **한 줄로 이어진다**: 마을 사람 다섯이 하나의 심부름으로 묶여 있다. 다음에 갈 곳은
  언제나 그 자리의 대사가 말하므로 지도를 헤맬 일이 없다.
- **본 만큼 달라진다**: 에필로그는 갈래가 아니라 **줄의 쌓임**이다. 등대에 불을 켰으면
  한 줄, 방을 잡았으면 한 줄, 조개 목걸이를 받았으면 한 줄이 붙어, 전부 본 플레이어는
  네 줄을 읽는다.
...
실제로 만들어진 맵(`tools/generate_port_maps.py`)의 구역은 다음과 같다. 기획 초안에서
몇 줄씩 당겨졌으므로 여기 적힌 것이 실물이다.

```
 y
 0      [테두리]   바위 테두리 한 칸 (좌우 끝 x=0, x=31도 같다)
 1~5    [울타리 너머]  숲으로 가는 길. 보이지만 갈 수 없다
 6      [북쪽 울타리와 문]  가로로 이어진 나무 울타리, 큰길 자리에 막힌 문
 7~13   [언덕]     등대(17~18, 10~12)와 등대지기(18, 13)
14~17   [돌계단]   언덕을 오르내리는 큰길 구간
18~26   [주택가]   빨래줄(12~13, 21), 살림집 둘, 아이가 배회하는 구역
27~32   [큰길가]   여관(12~14, 27~29)과 창고(18~20, 27~29)
33~40   [광장]     돌바닥(10~21, 34~39)과 그것을 두른 모래. 게시판, 우물, 좌판, 벤치
41      [물가]     모래에서 물로 내려가는 줄. 부두 판자만 지나갈 수 있다
42~47   [바다]     부두(15~17, 41~45)와 저녁 배(18~20, 43~44)
```

- 큰길은 x = 15, 16 두 칸이며 y 1에서 40까지 곧게 이어진다. **모든 목적지는 이 길에 접한다.**
- 바다는 y 41 아래 전부이며 통행 불가. 부두 판자만 통행 가능하다.
- 마을의 테두리는 **바위 한 칸**(x=0, x=31, y=0)이고, 북쪽은 y=6의 울타리가 막는다.
  문만 세우면 옆으로 걸어서 지나갈 수 있으므로 울타리를 가로로 이었다.
- 잔디와 모래가 만나는 자리는 타일셋의 가장자리 타일로 잇는다 (생성기의 후처리).
- 시작 위치는 부두 판자 위 (16, 43), 방향은 위쪽이며 **배에서 막 내린 자세**다.
...
| 좌표 | 오브젝트 | 트리거 | 반응 |
|---|---|---|---|
| (18, 44) | 배 | action | 최종 선택 (떠난다 / 더 둘러본다) |
| (16, 44) | 선장 | action / auto | 도착 인사(한 번)와 남쪽 항로 이야기 |
| (17, 42) | 밧줄 말뚝 | action | "굵은 밧줄이 말뚝에 단단히 매여 있다." |
| (14, 40) | 나무 상자 더미 | action | "누군가의 짐이다. 남쪽으로 간다는 표가 붙어 있다." **맵 파일에 실려 있다** |
| (13, 37) | 게시판 | action | 마을 공지 세 쪽 (배 시간표, 숲 출입 금지, 창고 열쇠) |
| (18, 36) | 우물 | action | "물이 차다. 두레박은 비어 있다." |
| (13, 35) | 생선 장수 | action | 창고의 사연과 등대 안내 (사슬의 첫 칸) |
| (19, 34) | 벤치 | action | "앉아 쉴 수 있을 것 같다." (앉는 연출은 범위 밖) |
| (13, 29) | 여관 문 | touch | 여관 1층으로 전환 (페이드, 문 소리) |
| (14, 29) | 여관 간판 | action | "항구 여관. 오늘도 방이 남아 있다." |
| (19, 29) | 창고 문 | action | 열쇠가 있으면 열리고 등유 한 통이 나온다. 없으면 잠김 (사연을 들었는지에 따라 대사가 바뀐다) |
| (12, 21) | 빨래줄 | action | "빨래가 바닷바람에 흔들린다." |
| (14, 20) | 아이 | action | 배회한다. 숲의 노래 이야기, 나중에 조개 목걸이 |
| (18, 13) | 등대지기 | action | 등유를 건네면 불을 켜고 은화 두 닢. 그 밖에는 선택지 둘 |
| (17, 12) | 등대 문 | action | 잠김. 불을 켜기로 한 뒤에는 대사가 바뀐다 |
...
| 아이템 | 어디서 | 무엇에 쓰는가 |
|---|---|---|
| 창고 열쇠 | 여관 주인 (생선 장수에게 창고 사연을 들은 뒤) | 창고 문 |
| 등유 한 통 | 창고 안 | 등대지기에게 |
| 은화 두 닢 | 등대지기 (등유의 사례) | 여관 방값 (실제로 치른다) |
| 조개 목걸이 | 아이 (등대지기의 천공의 끝 이야기를 들은 뒤) | 에필로그 한 줄 |
...
창고 사연을 듣고 왔으면 방 이야기보다 **열쇠 이야기가 먼저**다 (한 번만).

> **여관 주인**: 창고 얘기를 들으셨군요. 그 열쇠라면 여기 있어요. 주인이 떠나면서
> 맡기고 갔거든요.
> **여관 주인**: 등유가 남아 있을 거예요. 등대 노인께 가져다 드리세요. 주인도 그걸
> 아까워할 사람은 아니었어요.

선택지: **묵는다 / 배를 탄다 / 그냥 둘러본다** (취소는 3번)

- 묵는다 → **은화 두 닢이 있으면** "그럼 짐을 올려 두세요. 저녁 배는 놓치셔도 아침
  배가 또 옵니다." (`state.booked = true`, 은화 두 닢을 실제로 받아 간다)
- 묵는다 → **은화가 모자라면** "...두 닢이 모자라시네요. 괜찮아요, 이 마을에서 돈
  없다고 쫓겨난 사람은 없어요. / 일거리라면 언덕에 하나 있을 거예요." 막다른 길이
  아니라 **다음에 갈 곳을 가리킨다**
...
### 5.3 생선 장수 (광장 좌판 옆 13, 35, 고정, 얼굴 있음)
...
- 창고 → "저 창고요? 주인이 남쪽으로 떠난 지 삼 년째예요. 계약은 아직 살아 있어서
  ...
  → "열쇠는 여관 주인이 맡아 뒀어요. 안에 등유가 남아 있을 텐데, 등대 노인이 늘
  아쉬워하시죠." **사슬의 첫 칸이며, 다음에 갈 곳을 여기서 알려 준다**
- 볼 것 → "언덕에 등대가 있어요. 등대지기 노인이 계신데, 말은 별로 없어도
  물어보면 대답은 해 주세요."

### 5.4 아이 (주택가 13~18, 19~24 배회)
...
> **아이**: 등대 할아버지한테 물어봐 주세요. 그 할아버지는 뭐든 아니까.

등대지기에게 천공의 끝 이야기를 들은 뒤 (조개 목걸이를 준다):

> **아이**: 등대 할아버지도 노래 얘기를 했죠? 할아버지는 저를 안 놀려요.
> **아이**: 이거 드릴게요. 숲에서 주운 거예요. 배 타고 가다가 이거 보면, 여기
> 생각나실 거예요.

### 5.5 등대지기 (언덕 18, 13, 고정, 얼굴 있음)

**등유를 들고 가면 그 이야기가 먼저다** (사슬의 세 번째 칸).

> **등대지기**: ...그건 창고 것이군. 여관에서 열쇠를 받았나.
> **등대지기**: 고맙네. 오늘 밤은 아끼지 않고 켜겠네. 자네 배가 항구를 나갈 때까지는
> 밝을 걸세. (`state.lampReady = true`, 은화 두 닢을 준다)
> **등대지기**: 받아 두게. 여관 하루치일세. 쓸 일이 없으면 그냥 가지고 가고.
...
- 하늘 끝 → "제단이 있었네. 소리가 오래 남는 곳이었지. 그 뒤로는 배를 타지 않았네."
  → "숲에서 노래를 들었다는 아이가 있다지. 그 아이한테 거짓말쟁이라 하지 말게."
  (`state.heardAltar = true`. 선장의 대사가 바뀌고, 아이가 조개 목걸이를 준다)
- 등대 → "불을 켜는 일에는 오래고 짧고가 없네. 오늘 켜면 오늘의 일이지."
  → 아직 등유를 받지 못했으면 "다만 기름이 얼마 없네. 창고에 한 통 남아 있을 텐데,
  그 문이 삼 년째 잠겨 있어서." **사슬의 반대 방향 안내**

### 5.6 최종 선택 (부두의 배 18, 44)

`state.lampReady`가 없을 때:

> 저녁 배가 밧줄을 풀 준비를 하고 있다.
> 선택지: **지금 떠난다 / 조금 더 둘러본다**

`state.lampReady`가 있을 때:

> 저녁 배가 밧줄을 풀 준비를 하고 있다. 언덕의 등대에는 기름이 채워져 있다.
> 선택지: **지금 떠난다 / 불이 켜지는 것을 보고 간다**

에필로그는 **갈래가 아니라 줄의 쌓임**이다. 조건이 맞는 줄이 차례로 붙는다.

| 줄 | 조건 |
|---|---|
| "배는 저녁 물때에 항구를 떠났다." | 언제나 |
| "등 뒤에서 등대에 불이 켜졌다. 노인이 오늘 밤 몫의 기름을 다 붓는 모양이었다." | `lampReady` |
| "여관의 방 하나가 하룻밤 비어 있었다." | `booked` |
| "주머니 속에서 조개 목걸이가 달그락거렸다. 숲의 노래를 들었다는 아이는, 아직 그 마을에 있다." | 조개 목걸이를 가졌으면 |
...
| 소지품 창 | 화면 가운데, 목록과 설명 두 칸 | 취소키로 열고 닫는다. 열려 있으면 게임이 멈춘다 |

### 6.2 PC

- 키보드: 방향키 이동, Z/Enter/Space 결정, X 취소와 소지품, ESC 타이틀.
...
| 취소 버튼 | 결정 버튼 왼쪽 (248, 396) | 44x44 | 결정보다 작고 낮게. 한가할 때 누르면 소지품 창 |
...
| 여관 주인 | `message`, `choice` + 분기, `setFlag`, `if`(heardWarehouse / silver), `giveItem`, `takeItem` |
| 게시판 | `message` (두 쪽) |
| 여관 문 | `playSe`, `transfer` |
| 창고 문 | `if`(warehouse_key / openedWarehouse / heardWarehouse), `message`, `giveItem`, `playSe` |
| 아이 (배회) | `if`(shell / heardAltar), `message`, `giveItem` (배회는 이벤트 속성이지 커맨드가 아니다) |
| 등대지기 | `if`(lamp_oil), `message`, `choice` + 분기, `setFlag`, `takeItem`, `giveItem` |
| 최종 선택 | `if`(lampReady / booked / shell), `message`, `choice` + 분기, `scene`(fade) |
| 장소 이름 표시 | `showLocation` (맵 진입 auto 이벤트) |

**Lua 탈출구가 필요한 곳은 지금 기획에는 없다.** 그래도 남겨 둔다. 다음 로드맵의
전투나 미니게임은 커맨드로 적을 것이 아니기 때문이다.

10단계에서 커맨드가 둘 늘었다 (`giveItem`, `takeItem`)와 조건의 새 형태 `{ item = ... }`.
이것이 "그것 없이는 적을 수 없는 이벤트"의 실례다. 잠긴 문과 방값은 깃발만으로는
적을 수 없다.
...
- 시간대 변화(저녁이 되는 연출), 날씨
- 여관 2층, 방에서 자기, 저장과 로드
- ~~인벤토리와 은화~~ → **10단계에서 들어왔다.** 소지품 창이 있고, 여관 방값 은화 두
  닢은 실제로 치른다. 다만 물건을 **쓰는** 것(소모품, 장비)은 여전히 범위 밖이다
- 전투
