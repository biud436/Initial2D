...
|---|---|---|---|
| **튜닝 편차** | **-19.5센트** | **-19.5센트** | 모두 ±7센트 이내 |
| 스테레오 | 좁음 (corr 0.879) | 사실상 모노 (0.982) | 0.27~0.87 다양 |
| 저역 집중 | **90.1%** (<250Hz) | 85.5% (<250Hz) | 최대 75% |
| 장르 태그 | Rock | Rock | New Age/Game/Ballad |

가상악기는 항상 A440 정확히 맞는다. 두 록 트랙만 **같은 값으로 어긋난 튜닝**을 공유한다는
것은 귀로 조율한 동일한 기타(동일한 조율 상태)로, 아마 같은 시기의 세션에서 녹음되었다는
정황이다. 저자가 기억하는 "다른 제작 방식"이 측정치로 재구성된다: **시퀀싱이 아니라
연주를 녹음해 쌓는 방식.**
