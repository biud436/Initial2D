...
- **레이어 이름의 관례** (5단계와 10단계): `ground`(땅) → `deco`(캐릭터보다 아래에 그리는 것: 집 벽, 울타리, 우물) → `over`(캐릭터보다 위에 그리는 것: 빨래줄, 나무 윗동, 지붕 처마). 포맷이 강제하지는 않고 이름은 자유지만, 어디까지를 캐릭터 아래로 그릴지는 씬이 정한다 (`MapScene`의 `groundLayers`). 캐릭터 프레임이 타일보다 커서 머리가 윗 칸으로 올라가므로, 이 경계가 틀리면 벽 앞에 선 캐릭터의 머리가 벽에 가려진다.
