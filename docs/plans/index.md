...
| T1 | [t1-touch-input.md](t1-touch-input.md) | 모바일 터치 조작 (멀티터치, 화면 크기 대응) | Input 터치 API, `scripts/ui/layout.lua`, 조이스틱 vpad | **Fable 5** |
...
| T1. 모바일 터치 조작 | 🟡 진행 중 | 2026-08-24 | 구현과 검증 완료: 멀티터치 Input API(SDL 핑거 이벤트, 손가락별 4-상태 기계와 래치, 간접 터치 기기 필터), `scripts/ui/layout.lua`(화면 높이 비례 배치, 좁은 화면 축소), vpad 조이스틱화(손가락 소유권, 원 밖 유지), buttons 다중 포인터와 히트 슬롭 1.25배(겹침은 가까운 쪽이 이긴다). 알데바란 적용과 status() 배치 노출. 검수: Lua 단위 1673건(레이아웃 불변식 6개 해상도, 소유권, 동시 버튼, 재생기 터치), 엔진 씬 284건(인수 시나리오에 "패드로 달리면서 점프" 동시 입력 추가), 골든 무변경, APK 빌드 성공. **남은 것: S24 실기 확인** (무선 디버깅 재연결 대기) |
