...
다음 로드맵은 **필요가 생겼을 때** 시작한다. 지금 당장 v2를 열지 않는 이유는, 이번 로드맵의 목표가 "RPG를 만들 수 있는가"였고 그 답이 나왔기 때문이다. 아래는 순서가 아니라 후보 목록이며, 실제로 게임을 만들다가 막히는 곳이 다음 단계가 된다.
...
| 에디터의 이벤트 배치 | 지금은 좌표를 손으로 적는다 (3단계 마일스톤 3의 남은 몫) | InitialEditor + 브리지 + 맵 정의 파일 생성 | 중 |
