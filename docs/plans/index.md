...
| RPG 레이어 | `scripts/rpg/`의 캐릭터, 플레이어 입력, 카메라, 맵 씬, 시드 난수(5단계), 이벤트와 코루틴 실행기(6단계), 스킨 창과 대화창, 선택지(7단계), 리소스 고르기(8단계). 그리드 이동, y정렬 그리기, 트리거 4종, 맵 전환, 타자 효과와 얼굴 | 메뉴와 인벤토리 같은 그 밖의 창, 저장과 로드 (v2) |
...
## 6. 로드맵 완료와 다음 단계 (v2 초안, 2026-08-19)

여덟 단계가 모두 끝났다. 1절의 목표 문장은 데모 게임 "작은 마을"로 증명되었고, 그 증명은 사람이 눈으로 보는 것이 아니라 인수 시나리오(`tests/engine/scenes/rpgdemo_scene.lua`)가 매번 다시 확인한다.

**2026-08-19 갱신**: 이슈 24로 9단계([10-demo-v2.md](10-demo-v2.md))가 먼저 열렸다. "RPG를 만들 수 있는가"의 답은 나왔지만 "만든 것이 게임인가"는 아직이며, 그 답을 9단계가 맡는다. 아래 후보 중 **에디터의 이벤트 배치**와 그 전제인 이벤트 커맨드 데이터화는 9단계로 옮겨 갔다.

나머지는 순서가 아니라 후보 목록이며, 실제로 게임을 만들다가 막히는 곳이 다음 단계가 된다.

| 후보 | 왜 필요해지는가 | 어디에 붙는가 | 크기 |
|---|---|---|---|
| 저장과 로드 | `ctx.state`가 게임을 껐다 켜면 사라진다. 데모를 넘어가는 순간 첫 번째로 막히는 곳 | Lua (`ctx.state` 직렬화) + 이미 링크된 SQLite 또는 JSON 파일 IO | 중 |
| 메뉴와 인벤토리 | 아이템을 주는 이벤트는 있는데 가진 것을 볼 창이 없다 | Lua (`window.lua` 재사용), 데이터베이스 개념 필요 | 중 |
| 오토타일 | 맵을 손으로 그릴 때 가장 먼저 답답해지는 부분. 에디터와 엔진 양쪽 작업 | 맵 포맷 v2, 에디터, 타일맵 렌더러 | 대 |
| 씬 스택 | 맵 위에 메뉴를 겹치는 순간 단일 씬 교체 구조가 부족해진다 | C++ 씬 관리 또는 Lua 층 | 소 |
| 전투 시스템 | 장르를 넓힐 때. 범용 엔진 원칙상 전부 `scripts/rpg/` 아래 | Lua | 대 |
| 화면 효과 (날씨, 픽처, 색조) | 연출이 필요해지는 시점 | C++ 렌더러 보강 + Lua API | 중 |

원칙은 이번 로드맵과 같다. **"이 기능이 플래피버드에도 말이 되는가"**로 C++ 코어와 `scripts/rpg/`의 경계를 긋고, 단계마다 실행 가능한 결과물과 자동 검증을 남긴다.
