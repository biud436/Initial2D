...
//	}
//
//	// stb_image uses channels_in_file to tell us the actual number of channels in the image
//	int width, height, channels_in_file;
//	void* pvBits;
//
//	// Load the image - stb_image automatically handles all the PNG parsing
//	unsigned char* imageData = stbi_load(fileName.c_str(), &width, &height, &channels_in_file, STBI_rgb_alpha);
//
//	if (!imageData) {
//		MessageBox(g_hWnd, fileName.c_str(), "오류", MB_OK | MB_ICONERROR);
//		delete pTextureData;
//		return nullptr;
//	}
//
...
//	bmih.biSize = sizeof(BITMAPINFOHEADER);
//	bmih.biWidth = width;
//	bmih.biHeight = -height;  // Negative height for top-down bitmap
//	bmih.biPlanes = 1;
//	bmih.biBitCount = 32;     // Always 32-bit for RGBA
//	bmih.biCompression = BI_RGB;
//	bmih.biSizeImage = width * height * 4;
...
//	pTextureData->width = width;
//	pTextureData->height = height;
//	pTextureData->texture = CreateDIBSection(App::GetInstance().GetContext().mainContext,
//		bmi, DIB_RGB_COLORS, &pvBits, NULL, 0x0);
//
//	if (!pTextureData->texture || !pvBits) {
//		stbi_image_free(imageData);
//		delete pTextureData;
//		return nullptr;
//	}
//
//	// Convert RGBA to ARGB and copy to DIB
//	// stb_image loads in RGBA format, we need to convert to ARGB
//	for (int y = 0; y < height; y++) {
//		for (int x = 0; x < width; x++) {
//			unsigned char* pixel = imageData + (y * width + x) * 4;
//			((UINT32*)pvBits)[y * width + x] =
//				(pixel[3] << 24) |    // Alpha
//				(pixel[0] << 16) |    // Red
//				(pixel[1] << 8) |     // Green
//				(pixel[2]);           // Blue
